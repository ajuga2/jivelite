...
function event(self, event)
	...
	elseif type == ACTION then
		local action = event:getAction()
				-- RCS_V1 begin: capture volume_up here, but only if actionIndex > 1000 (see Framework.lua)
				--volume = math.floor(actionIndex/1000) - 1 (1 less so 0 is also allowed), agreed with external application
				--the action "volume_up" is arbitrary but must (of course) match with how the external application cummunicates its intent
				--via its udp packets to RemoteControlServer.lua
				--the packaging of the external info (an int value) into the actionIndex is done in Framework.lua
				--using custom actions like e.g. "volume_set" would have meant also changes to SlimBrowerApplet.lua and InputToActionMap.lua
				--uses argument directSet of _updateVolume to force volume
				log:info("action = ", action)
				local actionIndex = event:getActionInternal()
				log:info("actionIndex = ", actionIndex)

				if action == "volume_up" and actionIndex > 1000 then
					local volume = math.floor(actionIndex/1000) - 1
					log:info("volume = ", volume)
					self.delta = 0
					_updateVolume(self, nil, volume, true) --noAccel MUST be true otherwise it will not work as expected
					return EVENT_CONSUME
				end
				-- RCS_V1 end		
