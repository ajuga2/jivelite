...
function Digital:DrawTime(time)
	...
	-- RCS_V1 begin: override time if getTimerTime not nil
					local t = remoteControlServer:getTimerTime() -- returns {min=... , sec=...} or nil
					if t then
						time = { hour = t.min, min = t.sec} -- convert
					end
	-- RCS_V1 end
...
function _tick(self)
    local theTime = os.date(self.clock.clock_format)
			...
			local secs = remoteControlServer:getTimerSeconds() -- RCS_V1: for displaying timerSeconds, 
			if not secs then	-- RCS_V1: only if secs == nil
    if theTime == self.oldTime then
        -- nothing to do yet
        return
    end		
			end					-- RCS_V1: closing end 
...
function Digital:getDigitalClockSkin(skinName)
    ...
    elseif skinName == 'QVGA240squareSkin'  then
        local digitalClockBackground = Tile:loadImage(self.imgpath .. "Clocks/Digital/pir_clock_digital.png")
        local digitalClockDigit = {
            font = _font(90),
            fg = { 0xcc, 0xcc, 0xcc },
            w = WH_FILL,
...
function Analog:getAnalogClockSkin(skinName)
    ...
    elseif skinName == 'QVGA240squareSkin' then
        analogClockBackground = Tile:loadImage(self.imgpath .. "Clocks/Analog/pir_wallpaper_clock_analog.png")
