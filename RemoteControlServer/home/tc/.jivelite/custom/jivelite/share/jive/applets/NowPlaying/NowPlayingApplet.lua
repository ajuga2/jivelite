...
local function __getIcon(self, item, icon, remote) -- RCS_V1 original function _getIcon renamed to __getIcon
...
-- RCS_V1 begin: original function _getIcon replaced 
	local backupIcon = Icon("backupIcon") -- when timer has stopped this will be used once to fill the artwork icon
	local function _getIcon(self, item, icon, remote) 
		if remoteControlServer:getTimerTime() then 
			__getIcon(self, item, backupIcon, remote) 
		else -- use original function
			__getIcon(self, item, icon, remote) 
			backupIcon:setValue(icon:getImage()) 
		end
	end
-- RCS_V1 end
...
-- RCS_V1 begin: 
	local npSelf -- assigned in _createUI
-- RCS_V1 end
...
function _createUI(self)
	...
	log:info("new npSelf")
	npSelf = self -- RCS_V1 so it can be accessed by function timerHasUpdated below
	return window
end


-- RCS_V1 begin: 
	local function timerHasUpdated(timerSeconds, count)
		if npSelf == nil then return end -- not (yet) initiated
		if timerSeconds == nil then -- restore icon 
			log:info('restore artwork icon')
			npSelf.artwork:setValue(backupIcon:getImage())
			return
		end
		local function parseResolution(str)
			local w, h = str:match("(%d+)x(%d+)")
			return { w = tonumber(w), h = tonumber(h) }
		end
		local mmss = remoteControlServer:getTimerMMSS()
		local ARTWORK_SIZE = npSelf:getSelectedStyleParam('artworkSize')
		local resolution = parseResolution(ARTWORK_SIZE)
		local srf = Surface:newRGBA(resolution.w, resolution.h)
		local font = Font:load("fonts/FreeSans.ttf", math.floor(resolution.w/3))
		if timerSeconds > 0 or count % 2 == 1 then
			local srf2 = srf:drawText(font, 0xFFFFFFFF, mmss.mins .. ":" .. mmss.secs)
			log:info('draw time')
			npSelf.artwork:setValue(srf2)
		else -- 00:00 flashing
			npSelf.artwork:setValue(srf)
			log:info('draw empty')
		end
		log:info("timerHasUpdated " .. mmss.mins .. ":" .. mmss.secs)
	end

	remoteControlServer:addListener(timerHasUpdated)
-- RCS_V1 end
