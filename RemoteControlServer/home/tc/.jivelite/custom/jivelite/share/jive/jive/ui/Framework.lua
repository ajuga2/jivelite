...
function getActionEventNameByIndex(self, index)
		-- RCS_V1 begin: only the lower 3 decimal digits must be used, see NewActionEventRCS	
		index = index % 1000 
		-- RCS_V1 end
...
function newActionEvent(self, action) 
	...
	return Event:new(ACTION, actionIndex)
		...
		-- RCS_V1 begin
		-- extra int value as luggage
		-- index returned by _getActionEventIndexByName includes this extra int (x 1000)
		function newActionEventRCS(self, action, int) 
			if int == nil then int = 0 end -- int defaults to 0 when not present
			--first look for any action->action translation
			action = self:getActionToActionTranslation(action)
			local actionIndex = self:_getActionEventIndexByName(action)
			if not actionIndex then
			...
			local value = actionIndex + 1000 * int 
			log:info("new ActionEvent (", value, ") = ", actionIndex, " + 1000 x ", int )
			return Event:new(ACTION, value)
		end
		-- RCS_V1 end
